...
- **Platform `actual` implementations:**
  ...
  - `iosMain` — cinterop to `cpp/ios_bridge.cpp`, linking the static `libstockfish_ios.a` built by `compileIosNative`.
  - `wasmJsMain` — wraps a Web Worker running the `stockfish` engine, fetched at runtime from the unpkg CDN.

## Stockfish Source & Binaries

Stockfish is **compiled from source** for JVM, Android, and iOS via CMake. The source is downloaded from the official Stockfish GitHub releases (`sf_18`) by `downloadStockfishSource` and placed in `cpp/stockfish/`.
...
- iOS: static library `libstockfish_ios.a` compiled via CMake, linked through cinterop
- WASM: `stockfish-18.js`, `stockfish-18.wasm` fetched at runtime from the unpkg CDN
...
- Group: `fr.axl-lvy`
- Artifact: `stockfish-multiplatform` (and `stockfish-multiplatform-lite`)
- Version: see `library.version` in `gradle.properties`
